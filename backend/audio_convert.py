"""MP4 (AAC) audio output via PyAV (bundles its own FFmpeg libraries -- no
system ffmpeg binary required).

write_mp4 encodes a raw PCM numpy array directly to .mp4 -- this is what
generation uses now, so no intermediate .wav is ever written. wav_to_mp4
...
def write_mp4(samples: np.ndarray, sample_rate: int, mp4_path: str) -> None:
    """Encode a mono float32 PCM array (range [-1, 1]) directly to AAC/MP4."""
...
def wav_to_mp4(wav_path: str, mp4_path: str) -> None:
    """Legacy path: convert an existing .wav file to .mp4 (used only for
    history entries generated before write_mp4 existed)."""
    with av.open(wav_path) as in_container:
        in_stream = in_container.streams.audio[0]
        with av.open(mp4_path, mode="w") as out_container:
